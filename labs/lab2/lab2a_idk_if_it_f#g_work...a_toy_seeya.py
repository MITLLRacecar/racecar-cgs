...
RED = ((0, 100, 100), (179, 255, 255)) # HSV range for red
GREEN = ((36, 0, 0), (86, 255, 255)) # HSV range for green
# >> Variables
...
def update_contour():
    ...
    if image is None:
        ...
        if contours == rc_utils.find_contours(image, RED[0], RED[1]):
            ...
            if contours == rc_utils.find_contours(image, GREEN[0], GREEN[1]):
                # Select the largest contour
                contour = rc_utils.get_largest_contour(contours, MIN_CONTOUR_AREA)
                ...
                if contours == rc_utils.find_contours(image, BLUE[0], BLUE[1]):
                    # Select the largest contour
                    contour = rc_utils.get_largest_contour(contours, MIN_CONTOUR_AREA)
...
def start():
    ...
    if rc.controller.get_trigger(rc.controller.Trigger.RIGHT):
        ...
        speed=1
        angle=0
    if rc.controller.get_trigger(rc.controller.Button.A):
        print ("Speed:", speed, "Angle:", angle)
    if rc.controller.get_trigger(rc.controller.Button.B):
        print ("Contour center:", contour_center, "Contour area", contour_area)
